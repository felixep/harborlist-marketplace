...
create_test_users() {
    ...
    aws cognito-idp admin-create-user \
        ...
        --user-attributes Name=email,Value=individual@test.com Name=given_name,Value=John Name=family_name,Value=Doe Name=custom:customer_type,Value=individual Name=custom:tier,Value=individual \
    ...
    aws cognito-idp admin-create-user \
        ...
        --user-attributes Name=email,Value=dealer@test.com Name=given_name,Value=Jane Name=family_name,Value=Smith Name=custom:customer_type,Value=dealer Name=custom:tier,Value=dealer \
        --temporary-password "TempPass123!" \
        --message-action SUPPRESS
    ...
    aws cognito-idp admin-create-user \
        ...
        --user-attributes Name=email,Value=premium@test.com Name=given_name,Value=Bob Name=family_name,Value=Johnson Name=custom:customer_type,Value=premium Name=custom:tier,Value=premium \
        --temporary-password "TempPass123!" \
        --message-action SUPPRESS
    ...
    aws cognito-idp admin-create-user \
        ...
        --user-attributes Name=email,Value=superadmin@test.com Name=phone_number,Value=+1234567890 Name=given_name,Value=Super Name=family_name,Value=Admin Name=custom:role,Value=super-admin Name=custom:permissions,Value="*" Name=custom:team,Value=leadership \
        --temporary-password "TempPass123!@#" \
        --message-action SUPPRESS
    ...
    aws cognito-idp admin-create-user \
        ...
        --user-attributes Name=email,Value=admin@test.com Name=phone_number,Value=+1234567891 Name=given_name,Value=Admin Name=family_name,Value=User Name=custom:role,Value=admin Name=custom:permissions,Value="user_management,content_moderation,system_config,analytics_view" Name=custom:team,Value=operations \
        --temporary-password "TempPass123!@#" \
        --message-action SUPPRESS
...
EOF
    ...
    echo "✅ Environment configuration saved to ../backend/.env.local"
}
...
main() {
    ...
    echo "  ✅ Test users created for both pools"
    echo "  ✅ Environment configuration saved"
    echo ""
    echo "🚀 Next steps:"
    echo "  1. Start your backend services"
    echo "  2. Test authentication with the created test users"
    echo "  3. Verify group-based permissions are working"
    echo ""
