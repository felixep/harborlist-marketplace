...
import { Listing, BoatDetails, Location, Engine, EnhancedListing } from '@harborlist/shared-types';
 ...
 * @property {(listing: Omit<EnhancedListing, 'listingId' | 'createdAt' | 'updatedAt'>) => void} onSubmit - Form submission handler
 * @property {boolean} isLoading - Loading state for form submission
 * @property {Partial<EnhancedListing>} [initialData] - Initial form data for editing mode
 */
interface ListingFormProps {
  onSubmit: (listing: Omit<EnhancedListing, 'listingId' | 'createdAt' | 'updatedAt'>) => void;
  isLoading: boolean;
  initialData?: Partial<EnhancedListing>;
}
...
export default function ListingForm({ onSubmit, isLoading, initialData }: ListingFormProps) {
  const [formData, setFormData] = useState({
    ...
    boatDetails: initialData?.boatDetails || {
      ...
      beam: 0,
      draft: 0,
      engine: '', // Legacy field - kept for backward compatibility
      hours: 0, // Legacy field - kept for backward compatibility
      condition: 'Good' as const,
      engines: [], // New multi-engine support
      totalHorsepower: 0,
      engineConfiguration: 'single' as const
    },
    ...
    totalHorsepower: initialData?.totalHorsepower || 0,
    engineConfiguration: initialData?.engineConfiguration || 'single'
  });
  ...
  /**
   * Handle engine changes and update form data
   */
  const handleEngineChange = (engines: Engine[]) => {
  ...
  const handleMediaUpload = async (files: File[], type: 'images' | 'videos') => {
    ...
    try {
      ...
      const { uploadMultipleMedia, validateMediaFile } = await import('../../services/media');
      ...
      const uploadResults = await uploadMultipleMedia(files, (overallProgress, fileProgresses) => {
        console.log(`Upload progress for ${type}: ${overallProgress.toFixed(1)}%`);
        setUploadProgress(prev => ({
          ...prev,
          [`${type}_overall`]: overallProgress,
        }));
      });

      console.log(`Upload successful for ${type}:`, uploadResults);
      ...
      console.log(`Adding ${urls.length} ${type} URLs to form data:`, urls);
      setFormData(prev => {
        const updated = {
          ...prev,
          [type]: [...prev[type], ...urls],
          ...(type === 'images' && thumbnails.length > 0 && {
            thumbnails: [...prev.thumbnails, ...thumbnails]
          }),
        };
        console.log(`Updated form data:`, { images: updated.images, thumbnails: updated.thumbnails });
        return updated;
      });

      // Also store selected files for display purposes
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (uploadingFiles.images || uploadingFiles.videos) {
      alert('Please wait for media uploads to complete before submitting.');
      return;
    }
    ...
    if (formData.engines.length === 0) {
      alert('Please add at least one engine to your listing');
      return;
    }
    
    // Validate engines
    for (const engine of formData.engines) {
      if (!engine.horsepower || engine.horsepower <= 0) {
        alert(`Engine ${engine.position} requires a valid horsepower value`);
        return;
      }
    }
    
    // Create clean submission data for enhanced listing
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">Engine Configuration</h2>
        <EngineManager
          engines={formData.engines}
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        ...
        <MediaUpload
          ...
          uploading={uploadingFiles.images || uploadingFiles.videos}
          uploadProgress={uploadProgress['images_overall'] || uploadProgress['videos_overall'] || 0}
        />
      </div>
      ...
      <div className="flex justify-end space-x-4">
        ...
        <button
          type="submit"
          disabled={isLoading || uploadingFiles.images || uploadingFiles.videos}
